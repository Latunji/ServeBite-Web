...
<!-- Sidebar end=============================================== -->
		<div class="span9">		
			<div class="well well-small">
                    ...
                    <h4>Restaurants Around <c:forEach var="0" begin="1" end="${serviceProviders.length() - 1}">${serviceProviders.getJSONObject(0).getJSONObject("ciity").getString("name")}</c:forEach> </h4>
			  <ul class="thumbnails">
                              <c:forEach var="i" begin="0" end="${serviceProviders.length() - 1}">
				<li class="span3">
				  <div class="thumbnail">
                                      <a  href="product_details.html"><img src="<spring:url value="resources/img/tantalizers.png"/>" alt="" width="150" height="150"/></a>
					<div class="caption">
					  <h5>${serviceProviders.getJSONObject(i).getString("restaurantName")}</h5>
					  <p> 
						Phone Number : ${serviceProviders.getJSONObject(i).getString("phoneNumber")}
					  </p>
                                          <p> 
						Email : ${serviceProviders.getJSONObject(i).getString("email")}
					  </p>
					 
					  <h4 style="text-align:center"><a class="btn"> <i class="icon-zoom-in"></i></a> <a class="btn">View Meals <i class="icon-shopping-cart"></i></a> </h4>
					</div>
				  </div>
				</li>
                            </c:forEach>
    ...
    $("#submitButton").click(function(){
         ...
         let credentials = {
                    ...
                    success: function(response){
                        ...
                        if(response === "00") {
                            window.location.href = "${pageContext.request.contextPath}/refreshBal";
                        }
                        else if(response === "07"){
                            $("#spinner").html("Username/Password Does Not Exist!").addClass("text-danger");
                            $("#login_btn").attr("disabled", false);
                        }
                        else if(response === "01"){
                            $("#spinner").html("Invalid Password!").addClass("text-danger");
                            $("#login_btn").attr("disabled", false);
                        }
                        else{
                            $("#spinner").html("There was an error! Try again").addClass("text-danger");
                            $("#login_btn").attr("disabled", false);
                        } 
                    }
                });   
